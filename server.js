...
const server = http.createServer(async (req, res) => {
  ...
  if(req.method==='POST' && pathname==='/api/orders/create'){
    const body = await readBody(req);
    const { username, item, itemType, price, coins, houseId } = body;
    if(!username||!item){ fail(res,'Missing fields'); return; }
    try{
      const orderId = Date.now().toString(36).toUpperCase();
      await db.query(
        'INSERT INTO shop_orders (order_id, username, item, item_type, price, coins, house_id, status, created_at) VALUES(?,?,?,?,?,?,?,"pending",NOW())',
        [orderId, username, item, itemType||'', price||'', coins||0, houseId||null]
      );
    ...
    try{
      ...
      if(action === 'approve'){
        ...
        if(type === 'vip_bronze'){
          ...
          // houseId stored in order notes at purchase time
          const houseId = parseInt(order.house_id) || (body.houseId ? parseInt(body.houseId) : null);
          if(!houseId){ fail(res,'No House ID found on this order'); return; }
          const [[house]] = await db.query('SELECT id, owner, ownerid FROM houses WHERE id=? LIMIT 1',[houseId]);
          if(!house){ fail(res,'House ID '+houseId+' not found in database'); return; }
          if(house.ownerid && house.ownerid > 0){
